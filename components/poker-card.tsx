...
export function PokerCard({ value, selected, revealed, onClick, disabled }: PokerCardProps) {
  return (
    <button
      ...
      disabled={disabled}
      className={cn(
        'relative flex-1 min-w-0 max-w-[3.75rem] h-[3.5rem] sm:h-[5.25rem] rounded-lg sm:rounded-2xl flex flex-col items-center justify-center',
    ...
    >
      ...
      <span className="text-base sm:text-2xl font-bold tabular-nums">{value}</span>
